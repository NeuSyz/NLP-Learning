...
class BiLstmAttenModel(BaseModel):
    ...
    def build_model(self):
            ...
            with tf.name_scope("Bi-LSTM"):
                for idx, hidden_size in enumerate(self.config["hidden_sizes"]):
                    with tf.name_scope("Bi-LSTM" + str(idx)):
                        ...
                        outputs, final_state = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell,
                                                                               lstm_bw_cell,
                                                                               embedded_words,
        ...
        # 1.在Bi-LSTM+Attention的论文中，将前向和后向的输出相加
        # # 将最后一层Bi-LSTM输出的结果分割成前向和后向的输出
        # outputs = tf.split(embedded_words, 2, -1)

        # 2.实际用的话，可以直接用前向后向拼接的结果
        outputs = embedded_words  # [batch_size, time_step, hidden_size * 2]
        with tf.name_scope("Attention"):
            # H = outputs[0] + outputs[1]
            # 计算每个时序attention的权重得分
            atten_W = tf.layers.dense(outputs, 1, activation=tf.nn.tanh)  # [batch_size, time_step]
            atten_scores = tf.nn.softmax(atten_W, axis=1)
            # 得到Attention的输出
            atten_out = tf.squeeze(
                tf.matmul(tf.transpose(outputs, perm=[0, 2, 1]), atten_scores),
                axis=-1
            )
        ...
        with tf.name_scope("output"):
            ...
            self.logits = tf.nn.xw_plus_b(atten_out, output_w, output_b, name="logits")
