...
class DataHelper(object):
    ...
    def padding(self, inputs, sequence_length):
        ...
        """
        # 实际长度
        inputs_len = [len(sentence) if len(sentence) < sequence_length
                      else sequence_length for sentence in inputs]

        new_inputs = [sentence[:sequence_length]
                      if len(sentence) > sequence_length
                      else sentence + [0] * (sequence_length - len(sentence))
                      for sentence in inputs]
        assert len(inputs_len) == len(new_inputs)
        return new_inputs, inputs_len
    ...
    def gen_data(self):
        ...
        # 4，对输入做padding
        inputs_idx, inputs_len = self.padding(inputs_idx, self.config['sequence_length'])
        ...
        return np.array(inputs_idx), np.array(labels_idx), np.array(inputs_len)
    ...
    def to_tensor(self, data):
        ...
        :return:
        """
        x = torch.from_numpy(data).long().to(self.device)
        return x

    def next_batch(self, x, y, batch_size, x_len):
        """
        生成batch数据集
        :param x: 输入
        :param y: 标签
        ...
        for i in range(num_batches):
            ...
            batch_x_len = np.array(x_len[start: end], dtype="int64")

            batch_x = self.to_tensor(batch_x)
            batch_y = self.to_tensor(batch_y)
            batch_x_len = self.to_tensor(batch_x_len)
